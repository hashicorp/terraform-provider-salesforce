...
func (p profileType) GetSchema(_ context.Context) (schema.Schema, []*tfprotov6.Diagnostic) {
...
func (p profileDataSource) Read(ctx context.Context, req tfsdk.ReadDataSourceRequest, resp *tfsdk.ReadDataSourceResponse) {
	...
	if err := req.Config.Get(ctx, &pData); err != nil {
	...
	if err := resp.State.Set(ctx, &pData); err != nil {
		resp.Diagnostics = errToDiags(err)
	}
}
